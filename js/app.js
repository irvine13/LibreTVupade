...
+let userStatusElement = null;
+let userEmailElement = null;
+let logoutButton = null;
+
+// --- Main Content Elements (to hide/show) ---
+let searchArea = null;
+let resultsArea = null;
+let doubanArea = null;
+// Add other elements that need protection, e.g., history/settings buttons if needed
+
+// 页面初始化
+document.addEventListener('DOMContentLoaded', function() {
+    // --- Get Auth Element References ---
...
+    authErrorElement = document.getElementById('authError');
+    userStatusElement = document.getElementById('userStatus');
+    userEmailElement = document.getElementById('userEmail');
+    logoutButton = document.getElementById('logoutButton');
+
+    // --- Get Main Content Element References ---
+    searchArea = document.getElementById('searchArea');
+    resultsArea = document.getElementById('resultsArea');
+    doubanArea = document.getElementById('doubanArea');
+
...
+    // This function (defined in js/auth.js) will call our updateUIForAuthState function
+    onAuthStateChanged(updateUIForAuthState);
+
    // 初始化API复选框
    initAPICheckboxes();
    ...
    updateUIForAuthState(null); // Assume logged out initially
});
...
+        .then(userCredential => {
+            // Registration successful, onAuthStateChanged will handle UI update
...
+    logout()
+        .then(() => {
+            // Logout successful, onAuthStateChanged will handle UI update
+            console.log("登出成功");
+        })
+        .catch(error => {
...
+            return '密码强度不足，请使用更强的密码（至少6位）。';
+        case 'auth/operation-not-allowed':
+             return '邮箱/密码登录方式未启用。';
+        default:
+            return '发生未知错误，请稍后重试。 (' + error.code + ')';
+    }
+}
+
...
+        }
+        if (authContainer) authContainer.classList.add('hidden'); // Hide the whole auth box
+
+        // 显示需要登录的内容
+        if (searchArea) searchArea.classList.remove('hidden');
+        if (resultsArea) resultsArea.classList.remove('hidden'); // Or keep hidden until search
+        if (doubanArea) doubanArea.classList.remove('hidden'); // Or based on toggle
+        // Show other protected elements like history/settings buttons if they were hidden
+
+    } else {
+        // 用户未登录
+        if (authContainer) authContainer.classList.remove('hidden'); // Show the auth box
+        if (authForm) authForm.classList.remove('hidden');
+        if (userStatusElement) userStatusElement.classList.add('hidden');
+        if (authErrorElement) { // Clear any previous errors on logout/initial load
+             authErrorElement.textContent = '';
+             authErrorElement.classList.add('hidden');
+        }
+        if (authForm) authForm.reset(); // Clear form on logout
+
+        // 隐藏需要登录的内容
+        if (searchArea) searchArea.classList.add('hidden');
+        if (resultsArea) resultsArea.classList.add('hidden');
+        if (doubanArea) doubanArea.classList.add('hidden');
+        // Hide other protected elements
+
+        // Optional: Redirect to login or show a message if trying to access protected content while logged out
+    }
+}
+
+
