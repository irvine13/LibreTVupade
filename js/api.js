...
async function handleApiRequest(url) {
    ...
    try {
        if (url.pathname === '/api/search') {
            ...
            try {
                // --- Add Auth Header ---
                const idToken = await getIdToken(); // From js/auth.js
                const headers = { ...API_CONFIG.search.headers }; // Clone default headers
                if (idToken) {
...
async function handleAggregatedSearch(searchQuery) {
    ...
    const searchPromises = availableSources.map(async (source) => {
        try {
            ...
            // Ensure headers are correctly passed to fetch
            const fetchPromise = fetch(PROXY_URL + encodeURIComponent(apiUrl), {
                headers: headers // Use modified headers
                // signal: controller.signal // Timeout handled by Promise.race
            });
...
async function testSiteAvailability(apiUrl) {
    try {
        // --- Add Auth Header ---
        const idToken = await getIdToken();
        const headers = { 'Accept': 'application/json' }; // Keep Accept header for consistency if needed
        if (idToken) {
            headers['Authorization'] = `Bearer ${idToken}`;
        }
        // --- End Add Auth Header ---
        ...
        // 使用更简单的测试查询
        // Note: This still uses /api/search which might be blocked if not logged in.
        // Consider if this test needs to bypass auth or if failure is acceptable when logged out.
